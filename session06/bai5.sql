...
-- Tìm danh sách người dùng có tổng lượt thích của tất cả bài đăng > 20.
select u.user_id, username, count(post_id) as total_posts, sum(likes_count) as total_likes from Posts p
join Users u on p.user_id = u.user_id
group by user_id, username having total_likes > 20;
--  Tìm bài đăng có số lượt thích cao nhất của từng người dùng.
select u.user_id, username, post_id, content, max(likes_count) as max_likes from Posts p
join Users u on p.user_id = u.user_id
group by u.user_id, username, post_id, content having max_likes >= 20;
-- Tìm người dùng có ít nhất 2 bài đăng.
select u.user_id, username, count(post_id) as total_posts, sum(likes_count) as total_likes from Posts p
join Users u on p.user_id = u.user_id
group by u.user_id, username having total_posts >= 2;
-- Tìm bài đăng có số lượt thích ít nhất của từng người dùng.
select u.user_id, username, post_id, content, min(likes_count) as min_likes from Posts p
join Users u on p.user_id = u.user_id
group by u.user_id, username, post_id, content having min_likes < 15;
